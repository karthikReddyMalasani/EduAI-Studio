      ...
      mermaid.render(id, sanitizedChart).then(({ svg }) => {
        ...
        if (ref.current) {
          ref.current.innerHTML = `
            <div style="color:#ff6584; padding: 20px; font-size: 14px; text-align: center; border: 1px dashed rgba(255,101,132,0.3); border-radius: 10px;">
              <div style="font-weight:bold; margin-bottom: 8px;">📊 Diagram Rendering Error</div>
              <div style="font-family:monospace; font-size: 12px; opacity: 0.8;">${e.message || "Invalid Mermaid syntax generated by AI"}</div>
              <button onclick="window.location.reload()" style="margin-top:12px; padding: 5px 12px; background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: white; border-radius: 5px; cursor: pointer;">Retry Generation</button>
            </div>
          `;
        }
      });
